...
/**
 * Servicio para la gestión de los precios de los productos. Incluye la creación de nuevos precios,
 * cierre de precios anteriores y consulta de precios registrados.
 *
...
public class PrecioProductoService {
  ...
  @Autowired private PrecioProductoRepository precioRepo;

  /**
   * Crea un nuevo precio para un producto y cierra el precio anterior. El precio anterior se marca
   * como cerrado, estableciendo su fecha de fin en el día anterior al inicio del nuevo precio.
   *
  ...
  public PrecioProductoDTO crearNuevoPrecio(PrecioProductoDTO dto) {
    ...
    precioRepo
        .findByProducto_IdProductoAndFechaFinIsNull(dto.getIdProducto())
        .ifPresent(
            prev -> {
              prev.setFechaFin(
                  dto.getFechaInicio().minusDays(1)); // Marca el precio anterior como cerrado

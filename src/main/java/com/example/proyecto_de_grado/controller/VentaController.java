...
public class VentaController {
  ...
  public ResponseEntity<VentaDTO> crearVentaConDetalles(
      @Valid @RequestBody VentaConDetallesRequest requestBody) {
    try {
      // Validar que la request tenga datos válidos
      if (requestBody.getVenta() == null
          || requestBody.getDetalles() == null
          || requestBody.getDetalles().isEmpty()) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).build();
      }

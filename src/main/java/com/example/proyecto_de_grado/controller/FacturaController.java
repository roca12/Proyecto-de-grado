...
import com.example.proyecto_de_grado.service.FacturaService;
 ...
 *
 * <p>Esta clase maneja todas las operaciones HTTP relacionadas con la generación
 * de facturas en formato PDF. Se encarga de procesar las peticiones de facturación
 * y coordinar con los servicios correspondientes para generar documentos
 * profesionales de facturación.
 *
 * <p>Proporciona endpoints especializados para la generación de facturas
 * basadas en ventas registradas, permitiendo la descarga directa de
...
public class FacturaController {
     ...
     *
     * <p>Este endpoint consulta los datos de una venta y sus detalles asociados,
     * luego genera un documento PDF profesional que puede ser descargado
     * directamente por el cliente. La factura incluye toda la información
     * relevante de la transacción: productos, cantidades, precios y totales.
     *
     * <p>El archivo PDF generado se envía como respuesta HTTP con los headers
     * apropiados para forzar la descarga en el navegador del usuario.
     *
     * @param idVenta Identificador único de la venta para la cual generar la factura.
     * @return ResponseEntity con el archivo PDF como array de bytes y headers
     *         de descarga (HTTP 200), o HTTP 404 si la venta no existe,
     *         o HTTP 500 si ocurre un error en la generación.
     */
     ...
     *
     * <p>Similar al endpoint de descarga, pero configura los headers HTTP
     * para que el PDF se muestre directamente en el navegador en lugar
     * de forzar la descarga. Útil para vista previa de facturas.
     *
     * <p>Permite a los usuarios revisar el contenido de la factura
     * antes de decidir si descargarla o imprimirla.
     *
     * @param idVenta Identificador único de la venta.
     * @return ResponseEntity con el archivo PDF para visualización en línea
     *         (HTTP 200), o HTTP 404 si la venta no existe,
     *         o HTTP 500 si ocurre un error en la generación.
     */
    ...
    public ResponseEntity<byte[]> visualizarFactura(@PathVariable Integer idVenta) {
        try {
            ...
            System.err.println("Error inesperado generando factura para visualización: " + e.getMessage());
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
        }
    }
     ...
     *
     * <p>Este endpoint permite generar facturas sin necesidad de tener
     * una venta previamente registrada en la base de datos. Útil para
     * generar facturas de prueba o para sistemas que manejan datos
     * de venta externamente.
     *
     * <p>Recibe directamente los datos de venta y detalles en el cuerpo
     * de la petición HTTP y genera el PDF correspondiente.
     *
     * @param requestBody Objeto que contiene los datos de venta y detalles.
     * @return ResponseEntity con el archivo PDF generado (HTTP 200),
     *         o HTTP 400 si los datos son inválidos,
     *         o HTTP 500 si ocurre un error en la generación.
     */
    @PostMapping("/generar-personalizada")
    public ResponseEntity<byte[]> generarFacturaPersonalizada(@RequestBody FacturaPersonalizadaRequest requestBody) {
        try {
            ...
            List<DetalleVentaDTO> detalles = requestBody.getDetalles() != null ?
                    requestBody.getDetalles() : List.of();
     ...
     *
     * <p>Endpoint utilitario que valida si una venta específica contiene
     * toda la información necesaria para generar una factura válida.
     * Útil para validaciones del lado del cliente antes de intentar
     * generar el documento PDF.
     *
     * @param idVenta Identificador único de la venta a validar.
     * @return ResponseEntity con información sobre la validez de los datos
     *         (HTTP 200), o HTTP 404 si la venta no existe.
     */
    @GetMapping("/validar/{idVenta}")
    public ResponseEntity<ValidacionFacturaResponse> validarDatosFactura(@PathVariable Integer idVenta) {
        try {
            VentaDTO venta = ventaService.obtenerVentaPorId(idVenta);
            if (venta == null) {
                return ResponseEntity.notFound().build();
     ...
     *
     * <p>Utilizada para recibir datos de venta y detalles directamente
     * en el cuerpo de peticiones HTTP para generar facturas sin necesidad
     ...
     *
     * <p>Encapsula la información sobre la validez de los datos
     * de una venta para la generación de facturas.
     */

 ...
 *
 * <p>Este controlador proporciona endpoints para crear, leer, actualizar y eliminar
 * actividades, así como para listar actividades por finca con validación de permisos.</p>
 *
...
public class ActividadController {
     ...
     * <ul>
     *   <li>Los usuarios ADMIN pueden ver actividades de cualquier finca</li>
     *   <li>Otros usuarios solo pueden ver actividades de su finca asignada</li>
     * </ul></p>
     *
     * @param idFinca ID de la finca para filtrar actividades
     * @return ResponseEntity con la lista de actividades y estado HTTP 200 (OK),
     *         o estado 403 (Forbidden) si el usuario no tiene permisos
     * @throws RuntimeException si el usuario autenticado no se encuentra en la base de datos
     */
    @GetMapping("/finca/{idFinca}")
    public ResponseEntity<List<ActividadDTO>> obtenerActividadesPorFinca(@PathVariable Integer idFinca) {
        ...
        // Validar que el usuario no sea un ADMIN, y si no lo es, verificar que esté asociado a la finca correcta
     ...
     * @return ResponseEntity con la actividad encontrada y estado HTTP 200 (OK),
     *         o estado 404 (Not Found) si no se encuentra la actividad
     */
    @GetMapping("/{id}")
     ...
     */
    @PutMapping("/{id}")
    public ResponseEntity<ActividadDTO> actualizarActividad(@PathVariable Integer id, @RequestBody ActividadDTO dto) {

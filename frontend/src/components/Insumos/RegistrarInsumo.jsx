...
const RegistrarInsumo = () => {
  ...
  const handleRegister = async () => {
    if (!formData.nombre || !formData.unidadMedida || !formData.idProveedor) {
      setError(
        "Los campos Nombre, Unidad de Medida y Proveedor son obligatorios",
      );
      return;
    }
    ...
    try {
      ...
      if (error.message.includes("Duplicate entry")) {
        setError(
          "Ya existe un insumo con este nombre para este proveedor. Por favor, use un nombre diferente.",
        );
      } else {
        setError(error.message);
      }

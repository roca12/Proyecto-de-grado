...
const RegistrarCompra = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                if (!insumo) {
                    const insumoResponse = await fetch(`http://localhost:8080/insumos/${idInsumo}`);
    ...
    const handleSubmit = async (e) => {
        ...
        if (!formData.cantidad || !formData.precioUnitario || !formData.idProveedor) {
            setError("Todos los campos son obligatorios");
            return;
        }

        if (parseFloat(formData.cantidad) <= 0 || parseFloat(formData.precioUnitario) <= 0) {
        ...
        try {
            ...
            const fechaCompra = new Date(formData.fechaCompra).toISOString().split('T')[0];
        ...
        <div className="registro-compra-container">
            ...
            <form className="registro-form" onSubmit={handleSubmit}>
                ...
                <div className="form-group">
                    ...
                    {insumo.proveedor && (
                        <small>Proveedor actual: {insumo.proveedor.nombre} (no se puede modificar)</small>
                    )}
                </div>

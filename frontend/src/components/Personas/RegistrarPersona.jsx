...
const RegistrarPersona = () => {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorMessage = data.message || `Error al registrar ${tipoPersona}`;
        console.error("Error response:", data);
        throw new Error(errorMessage);
      }

      console.log("Success response:", data);
      setSuccess(`${tipoPersona === "cliente" ? "Cliente" : "Proveedor"} registrado exitosamente`);
      setTimeout(() => {
        navigate("/personas");
      }, 1500);
    } catch (error) {
      ...
      if (error.message.includes("Duplicate entry")) {
        if (error.message.includes("id_persona")) {
          setError("Esta persona ya está registrada como proveedor en el sistema. Por favor, use una identificación diferente.");
        } else {
          setError("Datos duplicados detectados. Por favor, verifique la información e intente nuevamente.");
        }
      } else {
        ...
        <div className="registro-persona-container" style={{ overflowY: "auto", maxHeight: "90vh" }}>
          ...
          <form className="registro-form" onSubmit={(e) => e.preventDefault()}>
            ...
            <div className="registro-botones">
              <button type="button" onClick={handleCancel} className="btn-cancelar" disabled={isLoading}>
                Cancelar
              </button>
              <button
                  type="button"

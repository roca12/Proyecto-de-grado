...
import {
    FaBars, FaTimes, FaUser, FaSignOutAlt, FaAddressBook,
    FaTruck, FaCheck, FaFile, FaCreditCard, FaChartArea, FaTable
} from "react-icons/fa";
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Actividades
                const actividadesRes = await fetch(`http://localhost:8080/actividades/finca/${currentUser.idFinca}`);
                const actividadesData = await actividadesRes.json();
                setActividades(actividadesData);

                // Producciones
                const produccionesRes = await fetch("http://localhost:8080/produccion", {
                    headers: {

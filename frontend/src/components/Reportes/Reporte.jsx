        ...
        const fetchData = async () => {
            try {
                ...
                const [
                    ...
                    authService.authFetch(`http://localhost:8080/produccion/finca/${idFinca}`),
                    authService.authFetch(`http://localhost:8080/actividades/finca/${idFinca}`),
                    fetch("http://localhost:8080/api/compra-insumos", {
                        headers: {
                            Authorization: `Bearer ${localStorage.getItem("authToken")}`
                        }
                    }),
                    authService.authFetch(`http://localhost:8080/api/ventas/finca/${idFinca}`),
                    fetch(`http://localhost:8080/insumos/finca/${idFinca}`),
                    authService.authFetch(`http://localhost:8080/api/clientes/finca/${idFinca}`),
                    authService.authFetch(`http://localhost:8080/api/proveedores/finca/${idFinca}`) // NUEVA PETICIÓN
                ]);
...
// PRIMERO obtener todos los datos
                ...
                const proveedores = proveedoresRes.ok ? await proveedoresRes.json() : []; // NUEVO

// DESPUÉS filtrar compras por proveedores de esta finca
                const comprasFiltradas = compras.filter(compra =>
                    proveedores.some(proveedor => proveedor.idProveedor === compra.proveedor?.idProveedor)
                );
    ...
    const clienteFrecuente = (() => {
        ...
        reportData.ventas.forEach((venta) => {
            ...
            if (idCliente) {
                ...
                const cliente = reportData.clientes?.find(c => c.idCliente === idCliente);
    ...
    const obtenerNombreCliente = (idCliente) => {
        ...
        return cliente ? `${cliente.nombre} ${cliente.apellido}` : `ID ${idCliente}`;
    };
    ...
    const procesarDatosVentas = (periodo) => {
        ...
        for (let i = periodos - 1; i >= 0; i--) {
            ...
            if (periodo === '3meses') {
                ...
                const fechaString = fecha.toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit' });
        ...
        for (let i = periodos - 1; i >= 0; i--) {
            ...
            if (periodo === '3meses') {
                ...
                const comprasDelMes = reportData.compras
                    ...
                    .reduce((sum, compra) => sum + ((compra.precioUnitario || 0) * (compra.cantidad || 1)), 0);
            ...
            if (periodo === '3meses') {
                ...
                const produccionDelMes = reportData.produccion
                    .filter(prod => {
                        ...
                        return fechaProd.getMonth() === fecha.getMonth() &&
    ...
    const procesarDatosActividades = (periodo) => {
        ...
        for (let i = periodos - 1; i >= 0; i--) {
            ...
            if (periodo === '3meses') {
                ...
                const actividadesDelMes = reportData.actividades
                    .filter(actividad => {
                        if (!actividad.fechaInicio) return false;
                        const fechaAct = new Date(actividad.fechaInicio);
                        return fechaAct.getMonth() === fecha.getMonth() &&
                ...
                const actividadesDelDia = reportData.actividades
                    .filter(actividad => {
                        if (!actividad.fechaInicio) return false;
                        const fechaAct = new Date(actividad.fechaInicio);
    ...
    const procesarDatosProveedores = () => {
        ...
        reportData.compras.forEach(compra => {
            ...
            const proveedor = reportData.proveedores?.find(p => p.idProveedor === compra.proveedor?.idProveedor);
            ...
            <div className="content-wrapper">
                ...
                <div className={`sidebar ${isOpen ? "open" : "collapsed"}`}>
                    ...
                    <div className="menu-items">
                        ...
                        <button onClick={() => navigate("/ventas")}><FaCreditCard /> {isOpen && "Ventas"}</button>
                    ...
                    <div className="reportes-container">
                        <div className="reportes-header">
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                <button className="btn-registrar" onClick={() => setVistaSeleccionada("simple")}>
                                    Vista Simple
                                </button>
                                <button className="btn-registrar" onClick={() => setVistaSeleccionada("avanzada")}>
                                    Vista Avanzada
                                </button>
                                <button className="help-button" onMouseEnter={() => setShowHelp(true)} onMouseLeave={() => setShowHelp(false)} onClick={toggleHelp}>
                                    ?
                                </button>
                                {showHelp && (
                                    <div className="help-tooltip">
                                        ...
                                        <ul>
                                            <li>
                                                <strong>Total Ventas:</strong> Suma del valor monetario de todas las ventas registradas.
                                                Muestra cuánto ha ingresado la finca por concepto de ventas.
                                            </li>
                                            <li>
                                                <strong>Total Compras:</strong> Suma del valor monetario de todas las compras de insumos.
                                                Representa cuánto ha gastado la finca en insumos.
                                            </li>
                                            <li>
                                                <strong>Producción Total:</strong> Cantidad total cosechada (en kg) de todos los cultivos
                                                en estado "COSECHADO". Indica la productividad de la finca.
                                            </li>
                                            <li>
                                                <strong>Actividades:</strong> Número total de actividades agrícolas registradas.
                                                Muestra la cantidad de trabajo realizado en la finca.
                                            </li>
                                            <li>
                                                <strong>Ganancia Estimada:</strong> Diferencia entre ventas y compras (Ventas - Compras).
                                                <span style={{color: '#4CAF50'}}> Verde</span> indica ganancias,
                                                <span style={{color: '#F44336'}}> rojo</span> indica pérdidas.
                                            </li>
                                            <li>
                                                <strong>Proveedor más usado:</strong> Proveedor del cual se han realizado más compras.
                                                Ayuda a identificar las relaciones comerciales más frecuentes.
                                            </li>
                                            <li>
                                                <strong>Cliente más frecuente:</strong> Cliente que ha realizado más compras.
                                                Identifica a los compradores más importantes.
                                            </li>
                                            <li>
                                                <strong>Insumo con mayor stock:</strong> Insumo que tiene la mayor cantidad disponible.
                                                Útil para gestionar inventarios y evitar sobreabastecimiento.
                                            </li>
                                            <li>
                                                <strong>Insumo con menor stock:</strong> Insumo con la menor cantidad disponible.
                                                <span style={{color: '#FF9800'}}>Amarillo/naranja</span> indica que puede necesitar reabastecimiento pronto.
                                            </li>
                                        </ul>
                                        <div style={{marginTop: '15px', padding: '10px', backgroundColor: '#f8f9fa', borderRadius: '5px'}}>
                                            <strong>Tip:</strong> Cambia a la vista avanzada para ver gráficos detallados y análisis temporales.
                                        </div>
                                    </div>
                        ...
                        {loading ? (
                            ...
                            <div className="summary-cards">
                                ...
                                <div className="card">
                                    <FaChartLine size={24} color={gananciaEstimada >= 0 ? "#4CAF50" : "#F44336"} />
                                    <h3>Ganancia Estimada</h3>
                                    <p style={{ color: gananciaEstimada >= 0 ? "#4CAF50" : "#F44336" }}>
                                        ${gananciaEstimada.toLocaleString()}
                                    </p>
                                    <small>{gananciaEstimada >= 0 ? "Utilidad" : "Pérdida"}</small>
                                </div>
                                <div className="card">
                                ...
                                <div className="grafica-container">
                                    <div className="grafica-header">
                                        <h3 className="grafica-title">
                                            <FaMoneyBillWave />
                                            Evolución de Ventas - {periodoVentas === '3meses' ? 'Últimos 3 meses' : 'Últimos 30 días'}
                                        </h3>
                                        <select
                                ...
                                <div className="grafica-container">
                                    <div className="grafica-header">
                                        <h3 className="grafica-title-compras">
                                            <FaShoppingCart />
                                            Evolución de Compras - {periodoCompras === '3meses' ? 'Últimos 3 meses' : 'Últimos 30 días'}
                                        </h3>
                                        <select
                                            className="periodo-selector"
                                    ...
                                    <ResponsiveContainer width="100%" height={300}>
                                        <LineChart data={procesarDatosCompras(periodoCompras)}>
                                            ...
                                            <YAxis tick={{ fontSize: 12 }} />
                                            <Tooltip
                                                formatter={(value) => [`$${value.toLocaleString()}`, 'Compras']}
                                            />
                                            <Line
                                                type="monotone"
                                                dataKey="valor"
                                ...
                                <div className="grafica-container">
                                    <div className="grafica-header">
                                        <h3 className="grafica-title-produccion">
                                            <FaLeaf />
                                            Evolución de Producción - {periodoProduccion === '3meses' ? 'Últimos 3 meses' : 'Últimos 30 días'}
                                        </h3>
                                        <select
                                            className="periodo-selector"
                                    ...
                                    <ResponsiveContainer width="100%" height={300}>
                                        <LineChart data={procesarDatosProduccion(periodoProduccion)}>
                                            <CartesianGrid strokeDasharray="3 3" />
                                            <XAxis
                                                dataKey="periodo"
                                                tick={{ fontSize: 12 }}
                                ...
                                <div className="grafica-container">
                                    <div className="grafica-header">
                                        <h3 className="grafica-title-actividades">
                                            <FaSeedling />
                                            Evolución de Actividades - {periodoActividades === '3meses' ? 'Últimos 3 meses' : 'Últimos 30 días'}
                                        </h3>
                                        <select
                                            className="periodo-selector"
                                    ...
                                    <ResponsiveContainer width="100%" height={300}>
                                        <LineChart data={procesarDatosActividades(periodoActividades)}>
                                            <CartesianGrid strokeDasharray="3 3" />
                                            <XAxis
                                                dataKey="periodo"
                                                tick={{ fontSize: 12 }}
                                ...
                                <div className="grafica-pastel">
                                    ...
                                    <div className="grafica-container">
                                        ...
                                        <ResponsiveContainer width="100%" height={400}>
                                            <BarChart data={procesarDatosInsumos()} margin={{ top: 20, right: 30, left: 20, bottom: 60 }}>
                                                <CartesianGrid strokeDasharray="3 3" />
                                                <XAxis
                                                    dataKey="nombre"
                                                    tick={{ fontSize: 11 }}
                                                ...
                                                <Bar dataKey="cantidad" fill="#FF9800">
                                                    {procesarDatosInsumos().map((entry, index) => (
                                                        <Cell
                                                            key={`cell-${index}`}
                                                            fill={entry.cantidad < 10 ? '#F44336' : entry.cantidad < 50 ? '#FF9800' : '#4CAF50'}
                                                        />
                                                    ))}
                                                </Bar>
                                            </BarChart>

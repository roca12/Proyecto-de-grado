...
const Ventas = () => {
        ...
        const fetchVentas = async () => {
            ...
            try {
                ...
                const [ventasData, clientesData, personasData] = await Promise.all([
                    fetchWithErrorHandling(`http://localhost:8080/api/ventas/finca/${idFinca}`, "Error al cargar ventas"),
                    fetchWithErrorHandling("http://localhost:8080/api/clientes", "Error al cargar clientes"),
                    fetchWithErrorHandling("http://localhost:8080/api/personas", "Error al cargar personas")
                ]);
                ...
                setError("Error al cargar los datos. Algunos datos pueden no estar disponibles.");
                setVentas([]);
                setClientes([]);
                setPersonas([]);
            } finally {
    ...
    const obtenerNombreCliente = (idCliente) => {
        if (!Array.isArray(clientes) || !idCliente) return `ID ${idCliente || 'N/A'}`;
        const cliente = clientes.find(c => c.idCliente === idCliente);
        return cliente ? `${cliente.nombre} ${cliente.apellido}` : `ID ${idCliente}`;
    };

    const obtenerNombrePersona = (idPersona) => {
        if (!Array.isArray(personas) || !idPersona) return `ID ${idPersona || 'N/A'}`;
        const persona = personas.find(p => Number(p.idPersona) === Number(idPersona));
        return persona ? `${persona.nombre} ${persona.apellido}` : `ID ${idPersona}`;
    };
            ...
            <div className="content-wrapper">
                <div className={`sidebar ${isOpen ? "open" : "collapsed"}`}>
                    ...
                    <div className="menu-items">
                        ...
                        <button onClick={() => navigate("/produccion")}><FaCheck /> {isOpen && "Produccion"}</button>
                ...
                <FaFile /> {isOpen && "Documentos"}
                        ...
                        <button onClick={() => navigate("/reportes-finca")}><FaChartArea /> {isOpen && "Reportes"}</button>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', position: 'relative' }}>
                                ...
                                {showHelp && (
                                    <div className="help-tooltip"
                                        ...
                                        <ul>
                                            <li><strong>Registrar Venta:</strong> Abre el formulario para crear una nueva venta.</li>
                                        </ul>
                                    </div>
                                )}
                            </div>

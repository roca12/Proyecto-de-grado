...
const RegistrarVenta = () => {
  ...
  useEffect(() => {
    ...
    const personaId =
      ...
      currentUser.id ||
      (tokenData &&
        (tokenData.idPersona || tokenData.idPersona || tokenData.id));
    ...
    if (!personaId) {
      ...
      setError(
        "Error crítico: No se pudo identificar al persona. Contacte al administrador.",
      );
      return;
    }
    ...
    const fetchData = async () => {
      try {
        ...
        // Cargar clientes
        const clientesRes = await authService.authFetch(
        ...
        // Cargar producciones
        const produccionRes = await authService.authFetch(
  ...
  const agregarDetalle = () => {
    ...
    const prod = producciones.find(
      (p) => p.idProduccion === parseInt(idProduccion),
    );
    if (!prod) {
    ...
    if (parseFloat(cantidad) > parseFloat(prod.cantidadCosechada)) {
      setError(
        `La cantidad no puede superar lo cosechado (${prod.cantidadCosechada} kg)`,
      );
      return;
    }
  ...
  const handleRegister = async () => {
    ...
    if (!formData.idPersona) {
      ...
      setError(
        "Error crítico: No se pudo identificar al persona. Recargue la página o contacte al administrador.",
      );
      return;
    }
    ...
    console.log(
      ...
      JSON.stringify(ventaPayload, null, 2),
    );
    ...
    try {
      ...
      const testRes = await authService.authFetch(
        "http://localhost:8080/api/ventas/test",
        {
          method: "POST",
          headers: {
      ...
      if (!testRes.ok) {
        throw new Error(
          `Test endpoint failed: ${testRes.status} - ${testText}`,
        );
      }

      // Now try the actual endpoint
      console.log(
        "Test passed, attempting actual endpoint: http://localhost:8080/api/ventas/con-detalles",
      );

      // Realizamos la petición usando authFetch para manejar autenticación
      const res = await authService.authFetch(
        "http://localhost:8080/api/ventas/con-detalles",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      try {
        ...
        console.log(
          "Could not parse success response as JSON, but request succeeded",
        );
      }
      ...
      try {
        for (const det of detalles) {
          ...
          let prod = producciones.find(
            (p) => p.idProduccion === det.idProduccion,
          );

          // 1a) Si no está en memoria, traerla del backend
          if (!prod) {
            const prodRes = await authService.authFetch(
              `http://localhost:8080/produccion/${det.idProduccion}`,
            );
            if (!prodRes.ok) {
              const errText = await prodRes
                .text()
                .catch(() => prodRes.statusText);
              console.error(
                `No se pudo obtener producción ${det.idProduccion}:`,
                errText,
              );
              continue;
            }
          ...
          if (nuevaCantidad < 0.000001) {
            console.warn(
              `Producción ${det.idProduccion}: la resta dejó 0 o negativo. Se pondrá en 0.`,
            );
          }
          ...
          if (!updateRes.ok) {
            const text = await updateRes
              .text()
              .catch(() => updateRes.statusText);
            console.error(
              `Error actualizando producción ${prod.idProduccion}:`,
              text,
            );
            continue;
          }
          ...
          console.log(
            `Stock actualizado para producción ${prod.idProduccion}: nueva cantidadCosechada = ${nuevaCantidad}`,
          );
        }
      } catch (err) {
        ...
        <form className="registro-form">
              ...
              >
                ...
                {producciones.map((p) => (
                  <option key={p.idProduccion} value={p.idProduccion}>
                    #{p.idProduccion} -{" "}
                    {p.fechaCosecha?.split("T")[0] || "sin fecha"}
          ...
          <div className="registro-botones">
            ...
            >
              {loading ? (
                <span>
                  Registrando... <i className="fa fa-spinner fa-spin"></i>
                </span>
              ) : (
                "Registrar Venta"
              )}
